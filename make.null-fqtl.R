...
in.dir <- argv[1]               # e.g., in.dir = '/broad/hptmp/ypp/ukbb/tempdata/1/9/'
...
################################################################
## 1. generate null data (theoretical null)
## 
## beta.hat ~ N(S R inv(S) * beta, S R S)
##            where beta = 0

svd.out <- take.ld.svd(X, options = list(do.stdize = TRUE))
...
dd <- svd.out$D
d <- nrow(dd)
theta.tilde <- sweep(.rnorm(d, N.TRAITS), 1, dd, `*`)
beta.null.mat <- (t(svd.out$V) %*% theta.tilde) * se.mat

## 2. estimate fQTL model
...
z.out <- fit.zqtl(effect = beta.null.mat, effect.se = se.mat,
